...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["application/octet-stream", "text/csv", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for charging-files" ON storage.objects FOR SELECT USING (bucket_id = 'charging-files');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for charging-files" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'charging-files');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for charging-files" ON storage.objects FOR UPDATE USING (bucket_id = 'charging-files');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for charging-files" ON storage.objects FOR DELETE USING (bucket_id = 'charging-files');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'charging-files',
        public: true,
        allowed_mime_types: ["application/octet-stream", "text/csv", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"],
